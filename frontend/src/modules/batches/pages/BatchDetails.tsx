...
export const BatchDetails: React.FC = () => {
  ...
  const addStudentMutation = useMutation({
    mutationFn: (studentId: string | number) => batchService.addStudent(id || '', studentId),
    onSuccess: () => {
  ...
  const removeStudentMutation = useMutation({
    mutationFn: (studentId: string | number) => batchService.removeStudent(id || '', studentId),
    onSuccess: (_, studentId) => {
      queryClient.invalidateQueries({ queryKey: ['batchStudents', id] });
      queryClient.invalidateQueries({ queryKey: ['batch', id] });
      if (selectedStudentId && String(selectedStudentId) === String(studentId)) {
  ...
  const filteredStudents = useMemo(() => {
    return mappedEnrolledStudents.filter(s => 
  ...
  const selectedStudentDetails = useMemo(() => {
    ...
    return mappedEnrolledStudents.find(s => String(s.id) === String(selectedStudentId)) || null;
  ...
  if (isLoadingBatch) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] py-12" id="batch-details-loading">
        <Loader2 className="h-10 w-10 animate-spin text-[#6C1D5F] mb-4" />
        <p className="text-sm font-medium text-[#6B7280]">Retrieving cohort metadata...</p>
      </div>
    );
  }
  ...
  if (isBatchError || !batch) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 text-center" id="batch-details-error">
        <AlertCircle className="mx-auto h-16 w-16 text-[#9CA3AF] mb-4" />
        <h3 className="text-xl font-semibold text-[#111827]">Failed to connect to backend service</h3>
        <p className="mt-2 text-sm text-[#6B7280] max-w-md mx-auto">
          We encountered an error retrieving details for Batch ID: "{id}". Please ensure the backend API server is fully running and active.
        </p>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8" id="batch-details-view">
      {/* Back Navigation Header */}
      <div className="mb-6 flex items-center justify-between" id="back-navigation-container">
        <button
          onClick={() => navigate('/teacher/batches')}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6 mb-8" id="batch-profile-header-card">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 pb-6 border-b border-[#E5E7EB]">
          <div className="space-y-2">
            ...
            <p className="text-sm text-[#6B7280]">
              Primary Teacher: <strong className="text-[#374151]">{batch.teacherName || 'Unassigned Teacher'}</strong>
            </p>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="text-center">
              <p className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wide">Roster Size</p>
              <p className="text-2xl font-bold text-[#111827] mt-1">
                {mappedEnrolledStudents.length} <span className="text-sm font-normal text-[#9CA3AF]">/ {batch.capacity || 30}</span>
              </p>
            </div>
            <div className="h-10 w-px bg-gray-200" />
            <div className="text-center">
              <p className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wide">Time Progress</p>
        ...
        <div className="mt-6" id="cohort-timeline-progress-bar">
          <div className="flex justify-between items-center text-xs font-medium text-[#9CA3AF] mb-1.5">
            ...
            <span className="font-semibold text-[#6C1D5F]">Timeline Elapsed</span>
          ...
          {[
            ...
            return (
              <button
                ...
                id={`tab-btn-${tab.id}`}
              >
                <Icon className={`h-5 w-5 ${isSelected ? 'text-[#6C1D5F]' : 'text-[#9CA3AF] group-hover:text-[#6B7280]'}`} />
                {tab.label}
              </button>
            );
          })}
      ...
      <div className="space-y-8" id="batch-tab-panel-container">
        ...
        {/* ==================== OVERVIEW TAB ==================== */}
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fadeIn" id="overview-tab-panel">
            {/* Stats Cards Section */}
            <div className="lg:col-span-2 space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5" id="overview-stats-grid">
                
                {/* Stat 1: Course Assigned */}
                <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Curriculum Program</span>
                  ...
                  <p className="text-xs text-[#9CA3AF] mt-2">Active syllabus for current cohort</p>
                </div>

                {/* Stat 2: Assignments Count */}
                <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Assignments Published</span>
                    <FileText className="h-5 w-5 text-[#6C1D5F]" />
                  </div>
                  <h4 className="text-2xl font-bold text-[#111827]">
                    {batchAssignments.length}
                  </h4>
                  <p className="text-xs text-[#9CA3AF] mt-2">Evaluations dispatched to students</p>
                </div>

                {/* Stat 3: Days Remaining */}
                <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Timeline Remaining</span>
                    <Clock className="h-5 w-5 text-yellow-600" />
                  </div>
                  <h4 className="text-2xl font-bold text-[#111827]">
                    {daysRemaining}
                  </h4>
                  <p className="text-xs text-[#9CA3AF] mt-2">Days left until cohort graduation</p>
                </div>

                {/* Stat 4: Cohort Capacity Fill */}
                <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Roster Fill Rate</span>
                    <Users className="h-5 w-5 text-green-600" />
                  </div>
                  <h4 className="text-2xl font-bold text-[#111827]">
                    {Math.round((mappedEnrolledStudents.length / (batch.capacity || 30)) * 100)}%
                  </h4>
                  <p className="text-xs text-[#9CA3AF] mt-2">Capacity threshold level allocation</p>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                ...
                <div className="flow-root">
                  <ul role="list" className="-mb-8">
                    {[
                      { id: 1, type: 'create', title: 'Batch Workspace Created', desc: `Learning Workspace for batch "${batch.name}" was successfully bootstrapped.`, date: 'Today at 9:30 AM', icon: CheckCircle, color: 'text-green-500 bg-green-50' },
                      { id: 2, type: 'syllabus', title: 'Curriculum Program Linked', desc: `Assigned course "${batch.courseTitle || 'Main syllabus'}" attached to workspace timeline.`, date: 'Yesterday at 4:15 PM', icon: BookOpen, color: 'text-[#6C1D5F] bg-[#F6F8FC]' },
                      { id: 3, type: 'roster', title: ' Roster Initialized', desc: `Pre-approved list of ${mappedEnrolledStudents.length} students synchronized with the batch.`, date: '2 days ago', icon: Users, color: 'text-[#6C1D5F] bg-[#F6F8FC]' }
                    ].map((act, actIdx) => (
                      <li key={act.id}>
                        <div className="relative pb-8">
                          ...
                          <div className="relative flex space-x-3">
                            <div>
                              <span className={`h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white ${act.color}`}>
                            ...
                            <div className="flex-1 min-w-0 pt-1.5 flex justify-between space-x-4">
                              <div>
                                ...
                                <p className="text-xs text-[#6B7280] mt-1">{act.desc}</p>
                              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                ...
                {batchAssignments.length === 0 ? (
                  <div className="text-center py-8 text-[#9CA3AF]">
                    ...
                    <p className="mt-2 text-xs font-medium">No assigned evaluations found</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {batchAssignments.slice(0, 3).map(asm => (
                      <div key={asm.id} className="p-4 rounded-xl bg-[#F6F8FC] border border-[#E5E7EB]">
                        <h4 className="text-sm font-semibold text-[#111827] truncate">{asm.title}</h4>
                        <div className="mt-2 flex items-center justify-between text-xs font-medium text-[#9CA3AF]">
                          <span className="flex items-center gap-1">
                            ...
                            Due: {asm.dueDate ? new Date(asm.dueDate).toLocaleDateString() : 'No Due Date'}
                          </span>
                          <span className="text-[#6C1D5F]">{asm.totalMarks} Marks</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {/* ==================== STUDENTS TAB ==================== */}
        {activeTab === 'students' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fadeIn" id="students-tab-panel">
            {/* Left Columns: Students Table */}
            <div className={`${selectedStudentId ? 'lg:col-span-2' : 'lg:col-span-3'} space-y-6`}>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-4">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl h-[44px] px-6 bg-[#6C1D5F] hover:bg-[#4A1E47] cursor-pointer"
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] overflow-hidden">
                <table className="min-w-full divide-y divide-[#E5E7EB]">
                  <thead className="bg-[#F6F8FC]">
                    <tr>
                      <th scope="col" className="py-3 px-4 text-left text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Student</th>
                      <th scope="col" className="px-3 py-3 text-left text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Primary Curriculum</th>
                      <th scope="col" className="px-3 py-3 text-left text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Attendance</th>
                      <th scope="col" className="px-3 py-3 text-left text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Avg Grade</th>
                          ...
                          >
                            <td className="whitespace-nowrap py-4 px-4 text-sm">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-semibold text-[#111827]">{student.fullName}</div>
                                  <div className="text-xs text-[#9CA3AF] font-medium">{student.email}</div>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (confirm(`Are you sure you want to dismiss ${student.fullName} from this cohort?`)) {
            ...
            {selectedStudentId && selectedStudentDetails && (
              <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6 space-y-6 self-start animate-slideLeft" id="student-details-drawer">
                <div className="flex items-center justify-between pb-4 border-b border-[#E5E7EB]">
                  <h3 className="text-base font-bold text-[#111827]">Roster Member Details</h3>
                  <button 
                ...
                <div className="text-center space-y-2">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-[#111827]">{selectedStudentDetails.fullName}</h4>
                    <p className="text-xs text-[#9CA3AF] font-medium">{selectedStudentDetails.email}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-[#F6F8FC] border border-[#E5E7EB]">
                  <div>
                    <p className="text-[10px] font-semibold text-[#9CA3AF] uppercase">Enrollment Status</p>
                    <p className="text-sm font-semibold text-green-600 mt-0.5">Active Member</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold text-[#9CA3AF] uppercase">Joined Cohort</p>
                    <p className="text-sm font-semibold text-[#111827] mt-0.5">{selectedStudentDetails.joinDate}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold text-[#9CA3AF] uppercase">Class Attendance</p>
                    <p className="text-sm font-semibold text-[#111827] mt-0.5">{selectedStudentDetails.attendance}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-semibold text-[#9CA3AF] uppercase">Current Grade</p>
                    <p className="text-sm font-semibold text-[#111827] mt-0.5">{selectedStudentDetails.grade}</p>
                  </div>
                </div>

                {/* Submissions Activity logs */}
                <div className="space-y-3">
                  <h5 className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Completed Assignments</h5>
                  <div className="space-y-2">
                    {[
                      { title: 'Core Concepts Quiz', score: '95/100', date: 'Yesterday' },
                      { title: 'Practical Lab 1', score: '18/20', date: '4 days ago' }
                    ].map((sub, sIdx) => (
                      <div key={sIdx} className="flex justify-between items-center text-xs p-2 bg-white rounded border border-[#E5E7EB]">
                        <div>
                          <p className="font-semibold text-[#111827] truncate max-w-[140px]">{sub.title}</p>
                          <p className="text-[10px] text-[#9CA3AF]">{sub.date}</p>
                        </div>
                        <span className="font-mono font-bold text-green-600">{sub.score}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-6 animate-fadeIn" id="assignments-tab-panel">
            ...
            <div className="flex justify-between items-center bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-4">
              <div className="text-sm text-[#6B7280] font-medium">
                Allocated Evaluations: <strong className="text-[#6C1D5F]">{batchAssignments.length} Assigned</strong>
              </div>
              <button
                onClick={() => navigate('/teacher/create-assignment')}
                className="inline-flex items-center rounded-xl h-[44px] px-6 bg-[#6C1D5F] hover:bg-[#4A1E47] cursor-pointer"
              >
            ...
            {batchAssignments.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-[#E5E7EB]">
                ...
                <h4 className="text-base font-semibold text-[#111827]">No active assignments allocated</h4>
                <p className="text-xs text-[#9CA3AF] max-w-sm mx-auto mt-1">
                  Assignments mapped to this cohort or associated syllabus curriculum will display here.
                </p>
                <button
                  onClick={() => navigate('/teacher/create-assignment')}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {batchAssignments.map((asm) => (
                  <div key={asm.id} className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6 flex flex-col justify-between space-y-4">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-mono font-semibold text-[#9CA3AF]">ID: {asm.id}</span>
                      </div>
                      <h4 className="text-base font-bold text-[#111827] line-clamp-1">{asm.title}</h4>
                      <p className="text-xs text-[#9CA3AF] line-clamp-2">{asm.description || 'No extended description provided.'}</p>
                    </div>
                    ...
                    <div className="border-t border-[#E5E7EB] pt-4 space-y-3">
                      <div className="flex items-center justify-between text-xs font-medium text-[#9CA3AF]">
                        <span className="flex items-center gap-1">
                          ...
                          Due: {asm.dueDate ? new Date(asm.dueDate).toLocaleDateString() : 'N/A'}
                        </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-base font-semibold text-[#111827]">Class Grade Profile</h3>
                    <p className="text-xs text-[#9CA3AF] mt-0.5">Average cohort score compared to baseline targets</p>
                  </div>
                  <TrendingUp className="h-5 w-5 text-[#6C1D5F]" />
                </div>
                
                {/* SVG Visual Bar Chart */}
                <div className="h-60 flex items-end justify-between px-4 pb-4 border-b border-l border-[#E5E7EB] relative" id="grades-profile-chart">
                  {/* Grid Lines */}
                  ...
                  {[
                    ...
                    <div key={bIdx} className="flex flex-col items-center gap-2 w-12 z-10">
                      <span className="text-[10px] font-bold text-[#6C1D5F]">{bar.val}%</span>
                      <div className={`w-8 rounded-t bg-[#6C1D5F] hover:bg-[#4A1E47] ${bar.height} transition-all duration-500`} />
                      <span className="text-[10px] text-[#9CA3AF] font-semibold">{bar.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-base font-semibold text-[#111827]">Submission Engagement Rate</h3>
                    <p className="text-xs text-[#9CA3AF] mt-0.5">Average turn-in rates for dispatched assessments</p>
                  </div>
                  <CheckCircle className="h-5 w-5 text-[#6C1D5F]" />
                </div>

                {/* SVG Visual Line Chart */}
                <div className="h-60 flex flex-col justify-between" id="submission-rate-chart">
                  <div className="flex-1 relative flex items-end">
                    <svg className="w-full h-full absolute inset-0 overflow-visible" preserveAspectRatio="none">
                      {/* Grid Lines */}
                      <line x1="0" y1="25%" x2="100%" y2="25%" stroke="#E5E7EB" strokeWidth="1" strokeDasharray="4 4" className="dark:stroke-gray-800" />
                      <line x1="0" y1="50%" x2="100%" y2="50%" stroke="#E5E7EB" strokeWidth="1" strokeDasharray="4 4" className="dark:stroke-gray-800" />
                      <line x1="0" y1="75%" x2="100%" y2="75%" stroke="#E5E7EB" strokeWidth="1" strokeDasharray="4 4" className="dark:stroke-gray-800" />
                  ...
                  <div className="flex justify-around items-center pt-4 border-t border-[#E5E7EB] text-xs mt-2">
                    <div className="text-center">
                      <p className="text-[#9CA3AF] font-medium">On-Time Submissions</p>
                      <p className="text-lg font-bold text-[#111827] mt-0.5">94.2%</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[#9CA3AF] font-medium">Resubmissions</p>
                      <p className="text-lg font-bold text-[#111827] mt-0.5">5.8%</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {isAddStudentOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto" id="enroll-student-modal">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div className="fixed inset-0 bg-[#111827]/40 backdrop-blur-sm transition-opacity" onClick={() => setIsAddStudentOpen(false)}></div>
            <div className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md">
              <div className="bg-white px-6 pb-6 pt-6 sm:p-8">
                <div className="flex justify-between items-center pb-4 mb-4 border-b border-[#E5E7EB]">
                  <h3 className="text-xl font-semibold text-[#111827]">Enroll Member</h3>
                  <button onClick={() => setIsAddStudentOpen(false)} className="text-[#9CA3AF] hover:text-[#6B7280]">
                    <X className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  <p className="text-xs text-[#6B7280]">
                    Select a registered student from the enterprise directory to enroll in this learning cohort.
                  </p>

                  <div>
                    <label htmlFor="student-selector-dropdown" className="block text-xs font-semibold text-[#9CA3AF] uppercase">Available Students</label>
                    <select
                    ...
                    >
                      ...
                      {availableStudents
                        .filter(student => !mappedEnrolledStudents.some((es: any) => String(es.id) === String(student.id)))
                        .map(student => (
              ...
              <div className="bg-[#F6F8FC] px-6 py-4 sm:flex sm:flex-row-reverse border-t border-[#E5E7EB]">
                ...
                >
                  {addStudentMutation.isPending ? 'Enrolling...' : 'Confirm Enrollment'}
                </button>
                <button
                  type="button"
